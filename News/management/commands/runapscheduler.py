...
def my_job():
    ...
    posts = Post.objects.filter(time_creation__gte=week_before)
    categories = set(posts.values_list('category__pk', flat=True))
    for cat in categories:
        if cat is not None:
            subscribers_mails = Subscription.objects.filter(category=cat).values_list('user__email', flat=True)
            subject = f'Weekly posts in {Category.objects.get(pk=cat)} category'
            posts_cat = posts.filter(category=cat)
            ...
            text_content = ("\n\n".join(
                [f"{post.title}\n Link:http://127.0.0.1:8000{post.get_absolute_url()}" for post in posts_cat]
            ))

            for email in subscribers_mails:
